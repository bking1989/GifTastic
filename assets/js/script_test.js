    ...
    function displayGIF() {
        ...
        $.ajax({
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                // faveBtn.addClass("btn btn-dark btn-sm text-white fav-btn");
                // faveBtn.attr("data-animate",results[i].images.fixed_height.url);
                // faveBtn.attr("data-still",results[i].images.fixed_height_still.url);
                // faveBtn.attr("data-url",JSON.stringify(results[i].bitly_url.replace(/['"]+/g, '')));
                // faveBtn.attr("data-rating",JSON.stringify(results[i].rating).toUpperCase().replace(/['"]+/g, ''));
                // faveBtn.attr("data-src",JSON.stringify(results[i].source_tld).replace(/['"]+/g, '')) || "Unknown";
            ...
            // When you click on 'Add to Favorites', the image is duplicated to the 'Favorites' section
            $(".fav-btn").on("click",function() {
                // Create a container for the favorite
                var newFave = $("<div>");
                newFave.addClass("faveContainer m-2");

                // Transfer the GIF image over to the favorite
                var faveImg = $("<img>");
                faveImg.addClass("favAni img-fluid");
                faveImg.attr("src",$(faveBtn).attr("data-still"));
                faveImg.attr("data-still",$(faveBtn).attr("data-still"));
                faveImg.attr("data-animate",$(faveBtn).attr("data-animate"));
                faveImg.attr("data-state","still");

                // Append the image and info to the container <div>
                $(faveImg).appendTo(newFave);
                $(newFave).appendTo("#faveDiv");

                // Function to animate the favorite image, when clicked
                $("img.favAni").on("click",function () {
